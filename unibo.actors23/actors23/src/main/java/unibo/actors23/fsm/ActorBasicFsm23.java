...
//    		  CommUtils.outgray("state ANNOT functor="+ functor + " p.length=" + p.length , ColorsOut.CYAN);
                    ...
                    for (int j = 0; j < nextStates.size(); j++) {
                        ...
                        if (hasInterrupt) {
                            if (!stateWithInterrupt) stateWithInterrupt = true;
                                ...
                                CommUtils.outred(stateName + ": multiple interrupt not allowed");
                            }
                        }
    ...
    protected void declareState(String stateName, StateActionFun action) {
        if (Actor23Utils.trace)
            CommUtils.outblue(getName() + " ActorBasicFsm23 | declareState " + stateName + " action=" + action);
 ...
 */
    ...
    protected void addTransition(String state, String msgId, boolean withInterrupt) {
        if (Actor23Utils.trace)
            CommUtils.outgray(getName() + " ActorBasicFsm23 | in " + curState + ": transition to " + state + " for " + msgId);
    ...
    */
    ...
    protected void elabMsg(IApplMessage msg) throws Exception {
        if (Actor23Utils.trace)
            CommUtils.outgray(getName() + " | ActorBasicFsm23 in " + this.curState + " elabMsg:" + msg);
        //Check for delegated
...
*/
    ...
    protected void resume() {
        if (Actor23Utils.trace)
            CommUtils.outgray(getName() + " | ActorBasicFsm23 in " + curState + " resume:" + memoTransTab.size());
