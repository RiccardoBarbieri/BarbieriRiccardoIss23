...
public class Proxy {
    ...
    public Proxy(String name, String host, String entry, ProtocolType protocol, IContext23 ctx) {
        try {
            if (Connection.trace)
                CommUtils.outgray(pfx + name + "  | CREATING entry= " + entry + " protocol=" + protocol);
            this.name = name;
            this.entry = entry;
    ...
    public void sendMsgOnConnection(String msg) {
        if (Connection.trace)
            CommUtils.outgray(pfx + name + "  | sendMsgOnConnection " + entry + " " + msg); //+ " conn=" + conn
        try {
            conn.forward(msg);
        } catch (Exception e) {
    ...
    protected void activateReceiver(Interaction conn) {
        new Thread() {
            public void run() {
                try {
                    if (Connection.trace)
                        CommUtils.outgray(pfx + name + " |  activateReceiver STARTED on conn=" + conn);
                    while (true) {
