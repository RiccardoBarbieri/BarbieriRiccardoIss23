...
public class Application1HTTPNoStop extends java.util.Observable {
    ...
    protected void walkByStepping(int n) throws Exception {
        ...
        if (n == 1) {
            updateObservers("robot-walking-column-sud");
        } else if (n == 2) {
            updateObservers("robot-walking-row-east");
        } else if (n == 3) {
            updateObservers("robot-walking-column-north");
        } else if (n == 4) {
            updateObservers("robot-walking-row-west");
        }
        while (goon) {
            goon = vr.step(250);
